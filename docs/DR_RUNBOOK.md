...
| **Exclusões de conta/conteúdo** | Contas excluídas nas últimas 24h **voltam ativas**; posts/comentários moderados/soft-deletados **reaparecem** | Consultar Sentry por `dr_audit:account_deletion` no período entre o backup e o disaster (espelho externo já implementado, sobrevive ao restore) + `deletion_tombstones` (se a migration `2026-09-17-dr-deletion-tombstone-and-integrity.sql` já tiver rodado antes do disaster), e reaplicar a exclusão manualmente (chamar `admin_delete_user`/soft-delete de novo para cada um). **Gap remanescente**: exclusão feita direto pela RPC a partir do portal vanilla (fora de `next-app`) não tem o espelho no Sentry — só os 2 caminhos de `next-app` (self-service e rota admin) capturam. |
...
| Exclusão de conta | Ação do usuário/admin, registrada em `audit_log` (mesma DB) + `deletion_tombstones` (mesma DB) + Sentry (`dr_audit:account_deletion`, EXTERNO) | Restore anterior à exclusão ressuscita a conta | `deletion_tombstones` dá o registro estruturado; Sentry sobrevive ao restore do banco — buscar `dr_audit:account_deletion` no período do restore e reaplicar MANUALMENTE. Não cobre exclusão via RPC direto do portal vanilla (ver Manual Action Required). | **Gap HIGH, mitigado** — reconciliação ainda manual, não automática |
| CSAM hash blocklist | `media_hash_blocklist` (mesma DB) + Sentry (`dr_audit:media_hash_blocked`, EXTERNO) | Restore anterior a um bloqueio reverte a proteção | Sentry tem o hash+categoria+timestamp de todo bloqueio feito pelo painel admin (`blockMediaPermanent`/`escalateToNcmec`) — buscar `dr_audit:media_hash_blocked` no período do restore e reinserir os hashes perdidos MANUALMENTE. | **CRITICAL, mitigado** — sinal externo existe, reconciliação ainda manual (nenhum job reaplica sozinho) |
...
- `/scripts/dr/pg-logical-backup.sh` — dump lógico manual do banco,
  independente do PITR gerenciado (rodar localmente com
  `SUPABASE_DB_URL`, nunca em CI — ver comentário no topo do arquivo).
- `next-app/lib/drAuditTrail.ts` — espelho externo (Sentry) de exclusão
  de conta e bloqueio de hash CSAM, usado na reconciliação pós-restore
  acima.
