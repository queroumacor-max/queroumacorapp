...
| **URGENTE**: `/migrations/2026-09-16-business-logic-security-audit.sql` (fix do CRÍTICO "PRO grátis pra sempre") sem confirmação registrada de execução | 🟡 SQL ESCRITO, EXECUÇÃO NÃO CONFIRMADA | Supabase SQL Editor — rodar a query de verificação do fim do arquivo |
| `/migrations/2026-09-17-rate-limit-sliding-window.sql` sem confirmação registrada de execução (risco aceito se não rodada, não regressão) | 🟡 SQL ESCRITO, EXECUÇÃO NÃO CONFIRMADA | Supabase SQL Editor |
| `/migrations/2026-09-18-final-pentest-hardening.sql` (4 achados novos do pentest final — ver entrada 2026-09-18) | 🟡 SQL ESCRITO E TESTADO CONTRA POSTGRES LOCAL, EXECUÇÃO EM PRODUÇÃO NÃO CONFIRMADA | Supabase SQL Editor — rodar a query de verificação do fim do arquivo |
