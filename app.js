...
function getProductImage(p){
  ...
  // Tintas Brazilian brand — real catalog photos, no size suffix
  const brMap = [
  ...
  // Detect container size from product name (for other brands)
...
async function loadPosts(feedIds){
  try {
    ...
    let commentsMap = {};
    const queries = [
      sb.from('profiles').select('id, name, tag, avatar_url, role').in('id', userIds),
