...
-- Idempotente: rodar 2x não duplica. Colar bloco por bloco se for no celular.

DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'brand_logos_image_url_https' AND conrelid = 'public.brand_logos'::regclass) THEN ALTER TABLE public.brand_logos ADD CONSTRAINT brand_logos_image_url_https CHECK (image_url ~* '^https://') NOT VALID; END IF; END $$;
