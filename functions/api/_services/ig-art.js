// @ts-check
// Business logic do gerador de arte pra Instagram.
// Não conhece Request/Response — recebe dados parseados, devolve resultado
// ou throw ServiceError. Controller cuida de auth/rate-limit/HTTP.
...
/**
 * Gera uma arte pra Instagram a partir de foto + estilo + aspect.
 * Throws ServiceError em validação ou falha de IA.
 * @param {{
 *   env: Record<string, string>,
 *   request: Request,
 *   photoDataUrl?: string,
 *   photoDataUrl2?: string,
 *   style?: string,
 *   aspect?: string,
 *   captionHint?: string,
 *   businessName?: string
 * }} args
 * @returns {Promise<{ imageDataUrl: string, caption: string, style: string, aspect: string, model: string }>}
 */
export async function generateIgArt({ env, request, photoDataUrl, photoDataUrl2, style, aspect, captionHint, businessName }) {
  if (!env.OPENAI_API_KEY) {
    throw new ServiceError('OPENAI_API_KEY não configurada no Cloudflare Pages', 503);
  }

  const styleKey = typeof style === 'string' ? style : 'portrait';
  const aspectKey = typeof aspect === 'string' && ASPECT_SIZES[aspect] ? aspect : 'square';
  const cleanHint = typeof captionHint === 'string' ? captionHint.trim().slice(0, 300) : '';
  const cleanBiz = typeof businessName === 'string' ? businessName.trim().slice(0, 80) : '';

  // Valida foto principal: data URL com base64
  const m = /^data:(image\/[a-z0-9+.-]+);base64,([A-Za-z0-9+/=\s]+)$/i.exec(photoDataUrl || '');
  if (!m) throw new ServiceError('photoDataUrl inválida — envie uma data URL de imagem', 400);
  const inputMime = m[1];
  ...
  const basePrompt = styleRef
    ? buildReferencePrompt(styleKey, cleanBiz, cleanHint, !!photo2) + ' ' + stylePrompt + aspectPart
  ...
  const [imgRes, capRes] = await Promise.all([
    ...
    generateCaption({ env, styleKey, captionHint: cleanHint, businessName: cleanBiz })
  ]);
...
function buildReferencePrompt(_styleKey, businessName, captionHint, hasPhoto2){
