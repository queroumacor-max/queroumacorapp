// @ts-check
// Business logic — moderação de texto + imagem via Google Gemini.
// Controller cuida de auth/rate-limit.
...
/**
 * @param {{ env: Record<string,string>, text?: string, imageUrl?: string }} args
 * @returns {Promise<{ flagged: boolean, severity: string, reasons: string[], categories: object, scores: object, engine: string, error?: string }>}
 */
export async function moderateContent({ env, text, imageUrl }) {
  const cleanText = typeof text === 'string' ? text.slice(0, 4000) : '';
  const imageUrlRaw = typeof imageUrl === 'string' ? imageUrl : '';
  const cleanUrl = (imageUrlRaw.startsWith('data:image/') || isAllowedImageHost(imageUrlRaw)) ? imageUrlRaw : '';

  if (!cleanText.trim() && !cleanUrl.trim()) {
    return { flagged: false, severity: 'none', reasons: [], categories: {}, scores: {}, engine: 'none' };
  }
  ...
  if (cleanUrl.trim()) {
    ...
    } catch { /* sem imagem inline, modera só o texto */ }
  }
  ...
  try {
    ...
    return { flagged, severity, reasons, categories: {}, scores: {}, engine: 'gemini' };
  } catch (err) {
    const isTimeout = err && (err.name === 'TimeoutError' || err.name === 'AbortError');
    if (isTimeout) throw new ServiceError('Gemini timeout (25s) — tente de novo', 504, { engine: 'failed' });
...
async function fetchImageInline(src) {
  ...
  const r = await fetch(src, { signal: AbortSignal.timeout(IMG_FETCH_TIMEOUT_MS) });
  if (!r.ok) return null;
