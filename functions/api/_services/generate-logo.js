// @ts-check
// Business logic — gera 3 variações de logo via OpenAI gpt-image-1.
import { ServiceError } from '../_security.js';

const TIMEOUT_MS = 45000;

function buildPrompts(name, styleHint) {
  return [
...
/**
 * @param {{ env: Record<string,string>, name?: string, style?: string }} args
 * @returns {Promise<{ urls: string[] }>}
 */
export async function generateLogo({ env, name, style }) {
  const cleanName = String(name || '').replace(/[^\p{L}\p{N}\s&\-.']/gu, '').trim().slice(0, 50);
  if (!cleanName) throw new ServiceError('name obrigatório', 400);
  const cleanStyle = String(style || '').replace(/[^\p{L}\p{N}\s,&\-.']/gu, '').trim().slice(0, 80);
  ...
  try {
    ...
    if (urls.some(u => !u)) throw new ServiceError('OpenAI não retornou imagem', 502);
    return { urls };
  } catch (e) {
    if (e instanceof ServiceError) throw e;
    const isTimeout = e && (e.name === 'TimeoutError' || e.name === 'AbortError');
    if (isTimeout) throw new ServiceError('DALL-E timeout (45s) — tente de novo', 504);
