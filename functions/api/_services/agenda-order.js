// @ts-check
// Business logic — otimiza ordem de visitas do dia.
...
/**
 * @param {{ env: Record<string,string>, date?: string, jobs: Array<any> }} args
 * @returns {Promise<{ ordered_ids: string[], notes: string }>}
 */
export async function orderAgenda({ env, date, jobs }) {
  const cleanDate = typeof date === 'string' ? date.slice(0, 10) : '';
  const rawJobs = Array.isArray(jobs) ? jobs.slice(0, 40) : [];
  const cleanJobs = rawJobs.map(j => ({
  ...
  if (cleanJobs.length < 2) throw new ServiceError('Envie ao menos 2 obras com id', 400);

  const validIds = new Set(cleanJobs.map(j => j.id));
  const userPrompt = `Data: ${cleanDate || '(sem data)'}\nObras do dia:\n` + cleanJobs.map(j =>
  ...
  const { text: raw, error } = await callAIText({
    env, systemPrompt: SYSTEM_PROMPT, userMessage: userPrompt,
    temperature: 0.2,
    maxTokens: 800,
    json: true
  });
