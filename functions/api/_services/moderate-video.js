// @ts-check
// Business logic da moderação de vídeo (Gemini + Supabase).
// Não conhece Request/Response — recebe parâmetros parseados, devolve
// resultado da decisão ou throw ServiceError. Controller cuida de HTTP +
// extração do JWT (auth + rate-limit ficam no controller porque dependem
// de Request).
...
/**
 * Valida sessão Supabase e retorna o userId.
 * Throw ServiceError em qualquer falha de validação.
 * @param {{ env: Record<string,string>, accessToken: string }} args
 * @returns {Promise<string>}
 */
export async function verifyOwnerToken({ env, accessToken }) {
  ...
  try {
    ...
    if (!uid) throw new ServiceError('token inválido', 401);
    return uid;
  } catch (e) {
...
/**
 * Modera um vídeo: valida ownership, baixa do storage do projeto, manda
 * pro Gemini, e aplica decisão (approve/reject/keep pending).
 * Devolve `{ status, reasons? }` no formato esperado pelo controller.
 * @param {{
 ...
 *   caption: string
 * }} args
 * @returns {Promise<{ status: 'approved'|'rejected'|'pending', reasons?: string[], reason?: string }>}
 */
export async function moderateVideoPost({ env, userId, postId, caption }) {
  const serviceKey = env.SUPABASE_SERVICE_ROLE || env.SUPABASE_SERVICE_KEY || env.SUPABASE_SERVICE_ROLE_KEY;
  const supaUrl = (env.SUPABASE_URL || FALLBACK_SUPABASE_URL).replace(/\/$/, '');
  const sHeaders = {
    'apikey': serviceKey,
  ...
  try {
    ...
    if (!arr?.[0] || arr[0].user_id !== userId) throw new ServiceError('não autorizado', 403);
  ...
  } catch { return { status: 'pending', reason: 'media_url inválida' }; }
  ...
  try {
    ...
    if (videoBuf.byteLength > MAX_BYTES) {
      return { status: 'pending', reason: 'vídeo grande — enviado para revisão humana' };
    }
  } catch (e) {
