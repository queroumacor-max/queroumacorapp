// @ts-check
// Business logic do resolve-color (deduz hex de cor de produtos de tinta).
...
/**
 * @param {{ env: Record<string,string>, items: Array<{id?:string, name?:string, code?:string}> }} args
 * @returns {Promise<{ colors: Record<string, string|null> }>}
 */
export async function resolveColors({ env, items }) {
  const arr = Array.isArray(items) ? items.slice(0, 60) : [];
